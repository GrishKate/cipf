...
class SwissRollSampler_discrete(Sampler):
    def __init__(
        self, dim=2, device='cuda'
    ):
        super(SwissRollSampler_discrete, self).__init__(device=device)
        assert dim == 2
        self.dim = 2
...
class Mix8GaussiansSampler_discrete(Sampler):
    def __init__(self, with_central=False, std=1, r=12, dim=2, device='cuda'):
        super(Mix8GaussiansSampler_discrete, self).__init__(device=device)
        assert dim == 2
        self.dim = 2
        self.std, self.r = std, r
